...
class TinkerAtroposTrainer:
    ...
    async def setup(self):
        ...
        if self.config.tinker.tinker_pretrained_weights is not None:
            self.training_client = await self.service_client.create_training_client_from_state_async(
                self.config.tinker.tinker_pretrained_weights
            )
            print(f"Training client created from pretrained weights: {self.config.tinker.tinker_pretrained_weights}")
        else:
            self.training_client = await self.service_client.create_training_client_from_state_async(
                base_model=self.base_model,
                rank=self.lora_rank,
            )
            print(f"Training client created from base model: {self.base_model} and LoRA rank: {self.lora_rank}")
